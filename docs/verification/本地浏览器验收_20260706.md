...
三个发现项已全部修复并浏览器复验通过（`16_market_fixed.png` / `17_risk_fixed.png`）：

1. **WS 鉴权支持 httpOnly cookie 回退**（后端根治）：`/ws` 与 `/ws/market` 在无 `?token=` 时读
   `ap_token` cookie——复验模拟"刷新后内存 token 丢失"场景，行情 WS 零 403、盘口正常
2. **下单按钮权限**：前端权限键 `trade.manual` 统一为后端权威键 `trade.manual_order`——
   admin 登录后按钮正确显示预检状态（"守卫未通过"因 SL 未填，语义正确）
3. **404 清零**：新增 `favicon.svg`（200）；补真实端点 `GET/PATCH /api/strategies`
   （受限策略集启停，**接入守卫第 11 项 strategy_enabled 真实生效**——被禁用模式的开仓会被拒，
   预检面板可见该项）——风控页策略卡真数据、控制台错误为空

![行情页修复后](img/16_market_fixed.png)
![风控页修复后](img/17_risk_fixed.png)

## 结论

- 11 个页面 + 2 个关键联动（HALTED/恢复）全部真实渲染，无白屏、无 JS 崩溃
- 后端 API/WS/事件/调度链路在真实浏览器场景全部工作（含本轮修复的实时推送与行情 WS 域名）
- ~~遗留 3 项~~ 已全部修复并复验（见「修复复验」小节）
