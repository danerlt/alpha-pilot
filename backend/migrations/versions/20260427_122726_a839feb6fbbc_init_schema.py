...
Revision ID: a839feb6fbbc
Revises:
Create Date: 2026-04-27 12:27:26.097329
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_agent_invocations_occurred_at", "agent_invocations", ["occurred_at"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "ix_factor_definitions_name_version", "factor_definitions", ["name", "version"], unique=True
    )
    op.create_table(
        "factor_snapshots",
        sa.Column(
    ...
    op.create_index(
        "ix_shadow_decisions_shadow_run_id", "shadow_decisions", ["shadow_run_id"], unique=False
    )
    op.create_table(
        "shadow_evaluations",
        sa.Column(
    ...
    op.create_index(
        "ix_trade_attributions_trade_id", "trade_attributions", ["trade_id"], unique=False
    )
    op.create_table(
        "trades",
        sa.Column(
    ...
    # Bootstrap default account/risk_profile (id=1) — 业务层多处 default=1
    # 依赖此行存在; 本次清掉 FK 后该约束在 DB 不再强制, 但业务期望仍为 id=1.
    op.execute(
