"""项目级硬编码常量集中地 (Plan 5 codereview Minor 项).

这里只放跨模块共享、且不该走 .env 配置的"硬上限"——
设计上是不可被运行时配置突破的安全闸门, 而非用户可调参数:

  - MAX_POSITION_SIZE_PCT_HARD_CAP : LLM 输出 position_size_pct 不论多大
                                      都被截到此值以下; spec §1.4 一票否决
  - CATCHUP_LIMIT_HARD_CAP         : GET /api/events/catchup 的最大 limit;
                                      防客户端要 100k 行把 DB 拖瘫
  - DEFAULT_EVENT_SHUTTLE_MAX_FAILED_ATTEMPTS :
                                      EventShuttle 死信阈值默认值, .env 可覆盖

各模块 `from src.shared.constants import X` 取值, 不再各自 hard-code.
"""
